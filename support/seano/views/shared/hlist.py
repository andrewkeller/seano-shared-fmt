...
def seano_cascade_hlist(notes, key):
    ...
    returned.
    '''
    def add_or_append(dst, obj, tag, level):
        ...
        if isinstance(obj, dict):
            for head, subheads in obj.items(): # dict used for head/subhead relationship
                node = add_or_append(dst, head, tag, level)
                for subhead in subheads:
                    add_or_append(node, subhead, tag, level + 1)
            return
        # Else, must be a str
        for x in dst['children']:
            if x['head'] == obj:
                x['tags'].append(tag)
                return x
        x = {'head': obj, 'tags': [tag], 'children': [], 'level': level}
    ...
    for n in notes:
        ...
        for bullet in (n.get(key, None) or {}).get('en-US', None) or []:
            add_or_append(lst, bullet, tag, 1)
    return lst['children']
